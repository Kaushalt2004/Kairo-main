...
bool LaneFollowPath::DecidePathBounds(std::vector<PathBoundary>* boundary) {
  ...
  // CARLA-Apollo: Add lateral tolerance to prevent spurious failures at turns
  // due to minor localization drift. The ego vehicle's SL lateral offset can
  // temporarily exceed the computed path boundary by a small amount when the
  // reference line has high curvature (e.g. turns), even when the vehicle is
  // correctly within its lane in Cartesian space.
  constexpr double kLateralTolerance = 0.5;  // metres
  if (init_sl_state_.second[0] > path_bound[0].l_upper.l + kLateralTolerance ||
      init_sl_state_.second[0] < path_bound[0].l_lower.l - kLateralTolerance) {
    AINFO << "not in self lane maybe lane borrow or out of road. init l : "
          ...
          << " ] (tolerance=" << kLateralTolerance << "m applied)";
    return false;
  }
